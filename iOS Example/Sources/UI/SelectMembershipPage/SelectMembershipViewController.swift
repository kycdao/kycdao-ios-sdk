...
class SelectMembershipViewController: UIViewController {
    ...
    private func updateTotalCost(years: UInt32) async {
        ...
        if estimation.paymentAmount > 0 {
            totalMembershipCost.text = estimation.paymentAmountText
        } else {
            totalMembershipCost.text = "Free"
        }
        
        if estimation.discountYears > 0 {
            discountYearsLabel.text = "Discounted years applied: \(estimation.discountYears)"
        } else {
            discountYearsLabel.text = "No discounts"
        }
